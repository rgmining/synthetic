...
class TestLoad(unittest.TestCase):
    ...
    def test_load(self):
        ...
        """
        graph = GraphMock()
        self.assertEqual(synthetic.load(graph), graph)

        anomalous = len([r for r in graph.reviewers if r.find("anomaly") > -1])
        self.assertEqual(anomalous, synthetic.ANOMALOUS_REVIEWER_SIZE)
